...
public static class HealthChecksExtension
{
    ...
    /// <remarks>
    /// Registers liveness (process up). Readiness checks (for example database) must be
    /// registered separately in the host application, tagged with "readiness".
    /// </remarks>
    /// <example>
    ...
    /// <remarks>
    /// Configures /health/live (liveness), /health/ready (checks tagged "readiness", e.g. database),
    /// and /health (all registered checks). Each endpoint uses the same JSON response writer.
    /// </remarks>
    /// <example>
    ...
    private static HealthCheckOptions WriteHealtCheckRespose(this WebApplication app, string tag)
    {
        ...
        {
            Predicate = check => string.IsNullOrEmpty(tag) || check.Tags.Contains(tag),
            ResultStatusCodes =
            {
